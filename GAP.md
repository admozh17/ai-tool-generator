...
| 10b | **View/eject the generated tool as code (engineer hand-off)** | ✅ Built | A "Code" tab serializes the composed spec into the `ToolSpec` JSON + a self-contained React/TS component an admin can copy to take over the prototype. Deterministic spec→code (not free-form codegen); role-aware (viewer's code omits write actions) and the emitted code calls the same governed endpoints, so masking/permissions stay server-side. |
